...
class craft_base_dataset(data.Dataset):
    ...
    def inference_pursedo_bboxes(self, net, image, word_bbox, word, viz=False):
        ...
        img_torch = torch.from_numpy(normalizeMeanVariance(input, mean=(0.485, 0.456, 0.406),
...
#             bboxes[j] = ori[:, :2]
    ...
    def saveInput(self, imagename, image, region_scores, affinity_scores, confidence_mask):
        ...
        boxes, polys = getDetBoxes(region_scores / 255, affinity_scores / 255, 0.7, 0.4, 0.4, False)
        ...
        target_gaussian_affinity_heatmap_color = cvt2HeatmapImg(affinity_scores / 255)
    ...
    def pull_item(self, index):
        ...
        image = normalizeMeanVariance(np.array(image), mean=(0.485, 0.456, 0.406),
                                              variance=(0.229, 0.224, 0.225))
...
class Synth80k(craft_base_dataset):
    ...
    def __init__(self, synthtext_folder, target_size=768, viz=False, debug=False):
        ...
        gt = sio.loadmat(os.path.join(synthtext_folder, 'gt.mat'))
