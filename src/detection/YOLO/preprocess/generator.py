...
class COCOGenerator:
    def __init__(self, label_dir, dataset_dir, mode, batch_size, input_shape, anchors, anchor_mask):
        ...
        self.img_metas = list()
        self.faulty_img = yaml.safe_load(open(os.path.join(WORKING_PATH, 'exclude.yml'), 'r'))['faulty_image']
    ...
    def set_filename(self):
        for img_meta in self.img_metas:
            img_meta['file_name'] = f'{self.dataset_dir}{img_meta["file_name"]}'

    def transform_targets(self, y_true, grid_size, anchor_idxs):
        y_outputs = np.zeros((grid_size, grid_size, anchor_idxs.shape[0], 6))

        for i in range(y_true.shape[0]):
            anchor_eq = np.equal(anchor_idxs, y_true[i][5])
            ...
            if np.any(anchor_eq):
                ...
                y_outputs[grid_xy[1]][grid_xy[0]][anchor_idx[0][0]] = [box[0], box[1], box[2], box[3], 1, y_true[i][4]]
    ...
    def transform_label(self, y_true):
        # y_train = [[x,y,w,h,c],...] shape=(n, 5)
        y_outs = []
        grid_size = math.ceil(self.input_shape[0] / 32)
        ...
        for anchor_idxs in self.anchor_mask:
            y_outs.append(self.transform_targets(y_train, grid_size, anchor_idxs))
            grid_size *= 2
    ...
    def set_labels(self):
        def fn(x): return self.ct.loadAnns(self.ct.getAnnIds(imgIds=x))
...
class SynthTextGenerator:
    def __init__(self, label_dir, dataset_dir, mode, input_shape, anchors, anchor_mask):
        self.data = sio.loadmat(label_dir)
        self.dataset_dir = dataset_dir
        self.mode = mode
        self.input_shape = input_shape
        self.anchors = anchors
