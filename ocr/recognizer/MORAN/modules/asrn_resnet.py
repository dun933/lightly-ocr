...
from .fractional_pickup import FractionalPickup
...
class AttentionCell(nn.Module):
    def __init__(self, input_size, hidden_size, num_embeddings=128, CUDA=True):
        ...
        self.fracPickup = FractionalPickup(CUDA=CUDA)
    ...
    def forward(self, prev_hidden, feats, cur_embeddings, test=False):
        ...
        prev_hidden_proj = self.h2h(prev_hidden).view(
            1, nB, hidden_size).expand(nT, nB, hidden_size).contiguous().view(
                -1, hidden_size)
        emition = self.score(
            torch.tanh(feats_proj + prev_hidden_proj).view(-1,
                                                           hidden_size)).view(
                                                               nT, nB)
        ...
        if not test:
            alpha_fp = self.fracPickup(
                alpha.transpose(
                    0, 1).contiguous().unsqueeze(1).unsqueeze(2)).squeeze()
            context = (feats * alpha_fp.transpose(0, 1).contiguous().view(
                nT, nB, 1).expand(nT, nB, nC)).sum(0).squeeze(0)  # nB * nC
            ...
            context = (feats * alpha.view(nT, nB, 1).expand(
                nT, nB, nC)).sum(0).squeeze(0)  # nB * nC
            if len(context.size()) == 1:
                context = context.unsqueeze(0)
            context = torch.cat([context, cur_embeddings], 1)
            cur_hidden = self.rnn(context, prev_hidden)
...
class ResNet(nn.Module):
    def __init__(self, c_in):
        ...
        self.block0 = nn.Sequential(nn.Conv2d(c_in, 32, 3, 1, 1),
