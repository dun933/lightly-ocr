...
def activation_fn(act, inplace=True):
    return nn.ModuleDict(
        [['relu', nn.ReLU(inplace=inplace)],
         ...
         ['selu', nn.SELU(inplace=inplace)], ['none', nn.Identity()]])[act]


class conv_bn(nn.Module):
    # apply 3x3 convolution with padding, added batchnorm and activation_fn of choice
    def __init__(self,
                 nIn,
                 nOut,
                 kernel_size=3,
...
class _resnet(nn.Module):
    def __init__(self, nIn, nOut, block_fn, layers):
        ...
        self.inplanes, self.out_blocks = int(
            nOut / 8), [int(nOut / 4),
                        int(nOut / 2), nOut, nOut]

        self.conv0_1 = conv_bn(nIn, int(nOut / 16))
        self.conv0_2 = conv_bn(int(nOut / 16), self.inplanes)

        self.maxpool1 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.block1 = self.stack_residual_block(block_fn, self.out_blocks[0],
                                                layers[0])
        self.conv1 = conv_bn(self.out_blocks[0], self.out_blocks[0])

        self.maxpool2 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
        self.block2 = self.stack_residual_block(block_fn, self.out_blocks[1],
                                                layers[1])
        ...
        self.maxpool3 = nn.MaxPool2d(kernel_size=2,
                                     stride=(2, 1),
                                     padding=(0, 1))
        self.block3 = self.stack_residual_block(block_fn, self.out_blocks[2],
                                                layers[2])
        self.conv3 = conv_bn(self.out_blocks[2], self.out_blocks[2])

        self.block4 = self.stack_residual_block(block_fn,
                                                self.out_blocks[3],
                                                layers[3],
                                                stride=1)
        self.conv4_1 = conv_bn(self.out_blocks[3],
                               self.out_blocks[3],
                               kernel_size=2,
                               stride=(2, 1),
                               padding=(0, 1))
        self.conv4_2 = conv_bn(self.out_blocks[3],
                               self.out_blocks[3],
                               kernel_size=2,
                               stride=1,
                               padding=0)
...
class ResNet(nn.Module):
    def __init__(self, nIn, nOut=512, net='resnet'):
        ...
        self._resnet = _resnet(nIn, nOut, residual_block, [1, 2, 5, 3])
