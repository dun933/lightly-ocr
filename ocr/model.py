...
class CRNNet(nn.Module):
    ...
    def __init__(self, config, device):
        ...
        if config['transform'] == 'TPS':
            self.Transformation = TPS_STN(F=config['num_fiducial'],
                                          im_size=(config['height'], config['width']),
                                          im_rectified=(config['height'], config['width']),
                                          num_channels=config['input_channel'],
                                          device=device)
        if config['backbone'] == 'ResNet':
            self.FeatureExtraction = ResNet50v2(config['input_channel'], config['output_channel'])
        else:
            raise Exception('No backbone module specified')
        # output: int(imgH/16-1) * 512
        self.AdaptiveAvgPool = nn.AdaptiveAvgPool2d((None, 1))  # Transform final (imgH/16-1) -> 1

        if config['sequence'] == 'biLSTM':
            self.SequenceModeling = nn.Sequential(BidirectionalLSTM(config['output_channel'], config['hidden_size'], config['hidden_size']),
                                                  BidirectionalLSTM(config['hidden_size'], config['hidden_size'], config['hidden_size']))
            self.sequenceOut = config['hidden_size']
        else:
            self.sequenceOut = config['output_channel']

        if config['prediction'] == 'CTC':
            self.Prediction = nn.Linear(self.sequenceOut, config['num_classes'])
        elif config['prediction'] == 'Attention':
            self.Prediction = Attention(self.sequenceOut, config['hidden_size'], config['num_classes'], device=device)
        else:
